...
public class BDMyLogic {
    ...
    public BDMyLogic(int in_size, MyPanel _master, int[] _colour_difference, int[] _colour_from, int[] _colour_black){
        ...
        kernel = new Kernel() {
            ...
            public void run() {
                ...
                BigDecimal bd_size = new BigDecimal(size);
                BigDecimal bd_mod = new BigDecimal(mod);
                BigDecimal bd_two = new BigDecimal(2);
                BigDecimal bd_x = new BigDecimal(x);
                BigDecimal bd_y = new BigDecimal(y);
                BigDecimal offset_x = new BigDecimal(offset[0]);
                BigDecimal offset_y = new BigDecimal(offset[1]);


                BigDecimal bd_za = bd_mod.multiply(offset_x.add(bd_x.subtract(bd_size.divide(bd_two))));
                BigDecimal bd_zb = bd_mod.multiply(offset_y.add(bd_y.subtract(bd_size.divide(bd_two))));
                BigDecimal bd_ca = bd_za.add(BigDecimal.ZERO);
                BigDecimal bd_cb = bd_zb.add(BigDecimal.ZERO);
                double za = bd_za.doubleValue();
                double zb = bd_zb.doubleValue();
                double ca = bd_ca.doubleValue();
                double cb = bd_cb.doubleValue();
